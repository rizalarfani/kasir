...
class Inventaris extends MY_Controller
{
	...
	public function __construct()
	{
		parent::__construct();
		$this->cek_login();
		$this->load->model(array('M_inventaris'));
	}

	private function meta($awal, $akhir)
	{
		$data = array(
			"data"          => $this->M_inventaris->get_data($awal, $akhir, $this->user_level, $this->user_id),
			"judul"			=> "Laporan",
	...
	{
		$data = array(
			"data"          => $this->M_inventaris->get_produk($this->user_addby),
			"dt" 			=> $this->M_inventaris->get_temp($this->user_id),
	...
	{
		$id = $this->input->get('id');
		$gudang = $this->M_inventaris->get_produk_akhir($this->user_addby, $id);
	...
	{
		$a = $this->M_inventaris->cek_stok($id, $mitra);
		$gudang = $this->M_inventaris->get_produk_akhir($this->user_addby, $id);
	...
	function add_post()
	{
		...
		$order_id = $this->M_inventaris->order_no($this->user_addby);
	...
	function data()
	{
		...
		$data = array(
			"data"          => $this->M_inventaris->get_data_kasir($awal, $akhir, $this->user_level, $this->user_id),
			"judul"			=> "Laporan",
			"keterangan"	=> ($awal == '') ? "Laporan pembelian barang hari ini" : "Laporan pembelian barang " . $awal . ' s/d ' . $akhir,
			"view"			=> "kasir",
		);
	...
	function detail($id)
	{
		...
		$data = array(
			"data"          => $this->M_inventaris->get_data_kasir_detail($id),
			"judul"			=> "Laporan",
