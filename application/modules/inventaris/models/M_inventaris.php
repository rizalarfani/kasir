...
class M_inventaris extends CI_Model
{
    ...
    {
        $this->db->select('stok_masuk.last_update,order_number,stok_masuk.id,stok_masuk.id_mitra,products.name,jumlah,user_namalengkap,(toko*jumlah) as jual');
        ...
        $this->db->order_by('stok_masuk.last_update', 'asc');
        if ($start_date == null) {
        ...
        $this->db->where('DATE(stok_masuk.last_update) BETWEEN "' . date('Y-m-d', strtotime($start_date)) . '" and "' . date('Y-m-d', strtotime($end_date)) . '"');
        $this->db->group_by('stok_masuk.id_mitra');
        $this->db->group_by('stok_masuk.order_number');

        return $this->M_Universal->getMulti(Null, "stok_masuk");
    }

    function get_data_kasir($start_date, $end_date, $level, $id)
    {
        $this->db->select('stok_masuk.*,user_namalengkap,sum(jumlah) as total ');
        $this->db->join('user', 'user.user_id = stok_masuk.toko_id', 'inner');
        $this->db->join('harga', 'harga.id = stok_masuk.id_harga', 'inner');
        $this->db->join('products', 'products.id = stok_masuk.product_id', 'inner');
   ...
   stok_masuk.id as msk_id,stok_out.id as out_id,
    ...
    {
        $this->db->select('stok_masuk.*,user_namalengkap, products.name as produk,description');
        $this->db->join('products', 'products.id = stok_masuk.product_id', 'inner');
        $this->db->join('user', 'user.user_id = stok_masuk.toko_id', 'inner');
        $this->db->order_by('stok_masuk.id', 'asc');
        return $this->M_Universal->getMulti(array('order_number' => $id), "stok_masuk");
    }
}
