...
class Pelunasan extends MY_Controller
{
	...
	public function __construct()
	{
		parent::__construct();
		$this->cek_login();
		$this->load->model(array('M_pelunasan'));
	}
	...
	{
		$data = array(
			"data"          => $this->M_pelunasan->get_data_it(),
			"judul"			=> "Laporan Pelunasan",
			"keterangan"	=> "Laporan pelunasan IT",
			"view"			=> "data_it",
		);
	...
	public function lihat($code)
	{
		...
		$data = array(
			...
			"toko"			=> $this->M_pelunasan->get_toko(dekrip($code), null),
			"view"			=> "detail",
		);
		$this->load->view('template', $data);
	}
	...
	function generate_it()
	{
		...
		$data = array(
			"data"          => $this->M_pelunasan->generate_it($akhir),
			"judul"			=> "Laporan",
			"keterangan"	=> "Laporan pembelian barang belum tergenerate s/d " . $akhir,
			"view"			=> "generate_it",
			"akhir"			=> $akhir,
	...
	function proses_generate($tgl)
	{
		...
		$toko = $this->M_pelunasan->generate_proses_toko($tgl);
		foreach ($toko as $t) {
			$data[] = $t->toko_id;
			$jumlah[] = $t->jumlah;
			$qty[] = $t->qty;
		...
		$toko = $this->M_pelunasan->generate_proses_manajemen($tgl);
		foreach ($toko as $t) {
			$data[] = 11; #id hfc
			$jumlah[] = $t->jumlah;
			$qty[] = $t->qty;
		...
		foreach ($ids as $id) {
			...
			$this->db->update('stok_out', $object);
		}

		$this->notification->success("Berhasil tambah !");
		redirect(base_url('pelunasan/lihat/' . enkrip($kode)));
	}
	...
	{
		$data = array(
			"data"          => $this->M_pelunasan->get_lunas_user($this->user_id),
			"judul"			=> "Laporan Pelunasan",
			"keterangan"	=> "Laporan pelunasan",
			"view"			=> "lunas_by_user",
		);
